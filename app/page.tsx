...
interface Set {
  id: number;
  setNumber: number;
  prescribed: { weight: number; reps: number; sets?: number };
...
export default function PTCommand() {
  ...
  const [exercises, setExercises] = useState<Exercise[]>(getTodaysExercises());
  ...
  const calculatePercentage = (weight: number, exercise: string) => {
    const exerciseKey = exercise.toLowerCase() as keyof OneRMs;
  ...
  // Update exercises when week or program changes
  useEffect(() => {
                  ...
                  <tr>
                    ...
                    {exerciseData.weeks.map((weekData, weekIndex) => (
                      <td key={weekIndex} style={{ 
                        ...
                        {calculatePercentage(weekData.weight, exerciseName)}%
                      </td>
                    ))}
                  </tr>
  ...
  return (
    <div style={{
      ...
      {currentTab !== 'today' && currentTab !== 'program' && (
